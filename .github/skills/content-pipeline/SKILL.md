...
> 默认已开启 `--inline` 模式（CSS 内联到 style 属性），适配微信 API 推送。如需本地预览可加 `--no-inline`。

推荐主题：`mozheng`（墨筝配色，默认）、`medium`（Medium 排版风格）、`chinese`（中国风）、`apple`（极简优雅）
推荐字号：`medium`（15px 默认）、`large`（16px 长文推荐）

**mozheng 主题说明**：「墨筝」公众号专属——墨色 #1C1C1E + 筝弦金 #C4956A + 宣纸底 #F2EDE3
...
**两套品牌色体系：**
- **墨筝（mozheng）**：内容品牌（公众号、深度文章、方法论）——墨色+筝弦金体系
- **鱼头头**：真人IP品牌（小红书、即刻、日常分享）——桃粉体系
...
**双品牌联动**：同一篇内容，公众号用墨筝色，小红书转发用鱼头头色
