---
title: 当我们在说 prompt cache 的时候我们在说什么
...
1. 存下之前已经算过的前置 Prompt 片段。
2. 遇到有同样片段的新请求，直接提出来用。
3. 这些被复用的 Token，结算时给你打个大折扣。
...
整个工作流程如下：
1. **第一次请求**：老老实实读完整个 Prompt，算完之后把这些“静态前缀”扔进缓存池。
2. **后来的请求**：
  - 模型一瞄，哎这前缀眼熟。
  - 直接把缓存里的 Token 序列调出来。
  - 算力这时候才启动，只去跑那些新加进去的动态 Token。

![](https://im.gurl.eu.org/file/AgACAgEAAxkDAAEBeEVpx5d9jvgSwjU8YHMrvWBbmzHuwwACOQtrGwTKQEZdWzLa0KuXqgEAAwIAA3cAAzoE.jpg)
...
**场景 1：无缓存裸奔**
- 请求成本 = (6,350 × $1.25/M) + (200 × $10/M) = **$0.00994 / 次**

**场景 2：跑了 Prompt Caching 后**
- 前缀缓存成本：6,000 × $0.125/M = $0.00075
- 动态处理成本：350 × $1.25/M = $0.00044
- 模型输出成本：$0.002
- 请求成本 = **$0.00319 / 次**
...
```json
{
  "model": "gpt-4.5", // 或你自己接的版本
  "prompt_cache_key": "k8s-docs-system-v1",
...
- **`prompt_cache_retention`**：要是你选了 `extended`（强力留存），对那种长尾流量多或者前缀特别长的业务就很友好。
