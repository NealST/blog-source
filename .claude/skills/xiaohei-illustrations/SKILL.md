...
description: 生成墨筝风格的小黑正文配图。用于用户要求为中文文章、帖子、博客、工作流文档、方法论、流程、结构、状态、隐喻或观点生成”怪诞””小黑””手绘””正文配图””文章插图””配图建议””shot list””去标题/改图”等任务；默认使用小黑 IP、纯白手绘、筝弦金为主强调色、简洁清爽但天马行空的视觉风格。
---
...
如果用户明确要求”生成 / 输出 / 做图 / 帮我生成”，不要停下来等确认；按 `references/prompt-template.md` 模板填充提示词，调用 TopView API 生图脚本逐张生成。不要把多张图拼在一张里。

**生成方式**：使用 content-pipeline 的 `topview_image_gen.py` 脚本调用 TopView AI API（GPT Image 2 模型，16:9，2K 分辨率）。

```bash
# 单张
python3 “${CONTENT_PIPELINE_DIR}/scripts/topview_image_gen.py” \
  --prompt “<填充后的完整提示词>” \
  --output “content/<slug>/xiaohei-illustrations/01-topic-name.png”

# 批量（先将 shot list 的 prompt 写入 JSON）
python3 “${CONTENT_PIPELINE_DIR}/scripts/topview_image_gen.py” \
  --batch shot-list.json \
  --output-dir “content/<slug>/xiaohei-illustrations/”
```

**batch JSON 格式**：
```json
[
  {“filename”: “01-topic-name.png”, “prompt”: “Generate one standalone 16:9 ...”},
  {“filename”: “02-topic-name.png”, “prompt”: “...”}
]
```
...
- 纯白背景
- 黑色手绘线稿
- 少量筝弦金（#A07848 白底加深版）/红色/蓝色中文手写批注
- 大量留白
- 小黑作为核心动作主体
...
配图直接生成到 content-pipeline 的产物目录：

```text
content/<slug>/xiaohei-illustrations/
```
...
生图依赖 TopView AI API，需要设置：

```bash
export TOPVIEW_API_KEY='your-api-key'
export TOPVIEW_UID='your-user-id'
```

脚本位置：`content-pipeline/scripts/topview_image_gen.py`（与 content-pipeline skill 共享）。
