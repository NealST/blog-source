...
**Problem:** When adapting from English source material, the AI word-for-word translates English metaphors and idiomatic expressions into Chinese, producing sentences that are grammatically correct but no native speaker would write. The result reads like translation software, not a person writing in Chinese. This also applies in the reverse direction (Chinese idioms calqued into English).
...
**Scope:** This pattern applies whenever writing in one language from source material in another. The examples here are English→Chinese, but the principle is symmetric.
...
**Problem:** The AI feels obligated to translate every sentence, every example, every paragraph from the source, preserving the source's structure even when the target language can make the same point more effectively with fewer words. The source's structure was designed for English readers and English rhythm. A Chinese writer making the same argument might use one idiom where the English used three examples, or cut an entire framing paragraph because the point is already obvious from context.

**Key insight:** Translation isn't just about converting words and adapting syntax. It's about knowing when the source's *structure* (number of examples, paragraph organization, level of explicitness) doesn't serve the target language. The best "translation" is sometimes radical compression: extract the core point, find the most natural Chinese way to say it, and drop everything the source used to *build toward* that point.
