...
| 风格 | 适用场景 | 产出 | 流程 |
|------|---------|------|------|
| **品牌信息图** | 流程、架构、因果、Pipeline、状态机 | `illustrations.html` → Playwright 截图 PNG | → 读 `references/cover-template.md` |
| **小黑手绘图** | 观点隐喻、认知锚点、前后对比、角色状态 | `xiaohei-illustrations/*.png`（AI 直出） | → 调用 `xiaohei-illustrations` skill |

**选型流程**（用户说"做配图"时执行）：
1. 消化正文，提炼需要配图的段落（认知锚点优先）
2. 对每张图判断：描述"怎么发生的"→ 品牌；描述"是什么/为什么"→ 小黑
...
按 `references/xiaohongshu-card-rules.md` 的完整规则执行内容拆分和卡片生成。核心要求：1:1 保留原文，禁止压缩。
